...
import { Package, Bot, CheckCircle, Leaf, Cloud, Recycle, Heart, ShieldCheck, RefreshCw, Star, Sparkles, AlertTriangle, ShoppingCart, Zap, FileText, CornerUpLeft } from 'lucide-react';
...
const fallbackListings: Listing[] = [
  ...
  { id: "202", product_id: "e2", seller_id: "s2", asking_price: 78000, condition_label: "Good", ai_grade: "Minor Wear", description: "Used 1 year for college. Screen pristine. Battery 187 cycles. Comes with charger.", is_available: true, product: { name: "Apple MacBook Air M2 13\"", category: "electronics", image_url: "https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=600&q=80" } },
  ...
  { id: "205", product_id: "c2", seller_id: "s5", asking_price: 5500, condition_label: "Good", ai_grade: "Light Use", description: "3 months gym use. Minimal sole wear. Boost cushioning responsive. UK9.", is_available: true, product: { name: "Adidas Ultraboost 22 Running Shoes — UK9", category: "clothing", image_url: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600&q=80" } },
  { id: "206", product_id: "c3", seller_id: "s6", asking_price: 1200, condition_label: "Like New", ai_grade: "Pristine", description: "Gifted, never worn. Tags attached. Off-white linen kurta + palazzo set. XL.", is_available: true, product: { name: "Fabindia Linen Kurta Set — XL", category: "clothing", image_url: "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=600&q=80" } },
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 flex flex-col md:flex-row gap-10">
        ...
        <div className="w-full md:w-[45%]">
          <div className="w-full aspect-square relative rounded-2xl overflow-hidden bg-slc-smoke border border-slc-divider">
            {listing.product.image_url ? (
              ...
              <div className="w-full h-full flex flex-col justify-center items-center text-slc-steel">
                <Package className="w-16 h-16 mb-2 text-slc-steel/50" />
                <span>No image</span>
              </div>
            )}
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <span className="bg-slc-cloud text-slc-ink px-4 py-2 rounded-full font-bold text-sm border border-slc-divider flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-slc-amber" /> {listing.ai_grade}
            </span>
            <span className="bg-purple-50 text-purple-700 px-4 py-2 rounded-full font-bold text-sm border border-purple-200 flex items-center gap-1.5">
              <Bot className="w-4 h-4" /> AI Verified
            </span>
          </div>

          <div className="bg-slc-leaf-light border border-slc-leaf rounded-xl p-4 mt-4">
            <p className="text-slc-ink text-sm font-semibold flex items-start gap-2">
              <CheckCircle className="w-5 h-5 text-slc-leaf shrink-0" /> 
              <span><span className="font-bold">SecondLife Certified:</span> This item was AI-inspected before listing. What you see is what you get.</span>
            </p>
          </div>
        </div>
        ...
        <div className="w-full md:w-[55%]">
          ...
          <div className="mt-2 text-slc-amber text-lg">
            <Star className="w-4 h-4 inline fill-current" /> <Star className="w-4 h-4 inline fill-current" /> <Star className="w-4 h-4 inline fill-current" /> <Star className="w-4 h-4 inline fill-current" /> <span className="text-slc-steel text-sm ml-1">4.2</span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <div className="bg-slc-leaf-light text-slc-leaf-dark text-xs font-semibold px-4 py-2 rounded-full border border-slc-leaf/30 flex items-center gap-1.5 shadow-sm">
              <Leaf className="w-4 h-4 fill-current" /> Earn {listing.green_credits_on_purchase || 150} Green Credits
            </div>
            <div className="bg-sky-50 text-sky-700 text-xs font-semibold px-4 py-2 rounded-full border border-sky-200 flex items-center gap-1.5 shadow-sm">
              <Cloud className="w-4 h-4 fill-current" /> Saves {listing.co2_saved || 2.1} kg CO₂
            </div>
          </div>
          ...
          <div className="mt-8 bg-slc-cloud rounded-xl p-4 border border-slc-divider">
            ...
            <div className="flex justify-between items-center relative max-w-sm mx-auto">
              ...
              <div className="flex flex-col items-center relative z-10 bg-slc-cloud px-2">
                <Package className="w-6 h-6 mb-1 text-slc-steel drop-shadow-sm" />
                <span className="text-[10px] font-bold text-slc-ink">Returned</span>
              </div>
              <div className="flex flex-col items-center relative z-10 bg-slc-cloud px-2">
                <Bot className="w-6 h-6 mb-1 text-slc-amber drop-shadow-sm" />
                <span className="text-[10px] font-bold text-slc-ink">AI Graded</span>
              </div>
              <div className="flex flex-col items-center relative z-10 bg-slc-cloud px-2">
                <Recycle className="w-6 h-6 mb-1 text-blue-500 drop-shadow-sm" />
              ...
              <div className="flex flex-col items-center relative z-10 bg-slc-cloud px-2 opacity-50 grayscale transition-all">
                <Heart className="w-6 h-6 mb-1 text-slc-leaf drop-shadow-sm fill-current" />
                <span className="text-[10px] font-bold text-slc-ink">Your Credits</span>
              </div>
            </div>
          </div>

          {/* Proactive Return Prediction Banner */}
          <div className="mt-8 bg-gradient-to-r from-orange-50 to-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3 items-start shadow-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 p-1 opacity-10">
              <RefreshCw className="w-24 h-24 text-amber-500 -mt-8 -mr-8" />
            </div>
            ...
            <div className="relative z-10">
              <h4 className="text-amber-900 font-bold text-sm flex items-center gap-2">
                Return Prediction <span className="bg-amber-200 text-amber-800 text-[10px] px-2 py-0.5 rounded uppercase tracking-wider font-extrabold">28% Risk</span>
              </h4>
              <p className="text-amber-800/80 text-xs mt-1.5 font-medium leading-relaxed">
                28% of buyers with your profile return this size in {listing.product.category.toLowerCase()}. Are you sure about the fit?
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3">
            <button 
              ...
              className="bg-slc-amber hover:bg-yellow-500 text-slc-ink font-bold py-4 rounded-full text-base w-full shadow-sm transition-colors flex items-center justify-center gap-2"
            >
              <ShoppingCart className="w-5 h-5" /> Add to Cart
            </button>
            <button 
              onClick={handlePurchase}
              className="bg-[#FFA41C] hover:bg-[#FA8900] text-slc-ink font-bold py-4 rounded-full text-base w-full shadow-sm transition-colors flex items-center justify-center gap-2"
            >
              <Zap className="w-5 h-5" /> Buy Now
            </button>
            <button
              onClick={() => router.push(`/verify/${params.id}`)}
              className="w-full border border-slc-leaf/30 text-slc-leaf text-sm font-bold py-3 rounded-xl hover:bg-slc-leaf-light transition-colors mt-2 flex items-center justify-center gap-2"
            >
              <FileText className="w-4 h-4" /> View Full Health Card & Certificate
            </button>
            <button 
              onClick={() => router.back()}
              className="text-slc-steel text-sm text-center mt-2 hover:underline cursor-pointer font-medium flex items-center justify-center gap-1.5 mx-auto"
            >
              <CornerUpLeft className="w-4 h-4" /> Return to Marketplace
            </button>
          </div>

            <div className="grid grid-cols-2 gap-4 mt-6">
              <div className="flex flex-col items-center text-center p-3 border border-slc-divider rounded-xl">
                <ShieldCheck className="w-6 h-6 mb-1 text-slc-steel" />
                <span className="text-xs font-bold text-slc-ink">Amazon Protected</span>
                <span className="text-[10px] text-slc-steel">Secure checkout</span>
              </div>
              <div className="flex flex-col items-center text-center p-3 border border-slc-divider rounded-xl">
                <RefreshCw className="w-6 h-6 mb-1 text-slc-steel" />
                <span className="text-xs font-bold text-slc-ink">7-Day Returns</span>
                <span className="text-[10px] text-slc-steel">No questions asked</span>
              </div>
            </div>
